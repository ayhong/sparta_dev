...
Collide::Collide(SPARTA *sparta, int, char **arg) : Pointers(sparta)
{
  ...
  // collision limiter off by default
  clim = -1;
...
void Collide::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"vremax") == 0) {
      ...
      iarg += 3;
    } else if (strcmp(arg[iarg],"climit") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal collide_modify command");
      clim = atof(arg[iarg+1]);
      if (clim < 0) error->all(FLERR,"Illegal collide_modify command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"bgk_mod") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal collide_modify command");
      if (strcmp(arg[iarg+1],"BGK") == 0) bgk_mod = 1;
      else if (strcmp(arg[iarg+1],"ESBGK") == 0) bgk_mod = 2;
      else error->all(FLERR,"Illegal collide_modify command");
      nattempt_lim = atof(arg[iarg+2]);
      if (nattempt_lim < 0) error->all(FLERR,"Illegal collide_modify command");
      mu_ref = atof(arg[iarg+3]);
      if (mu_ref < 0) error->all(FLERR,"Illegal collide_modify command");
      iarg += 4;
    } else if (strcmp(arg[iarg],"Pr") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal collide_modify command");
      Pr_in = atof(arg[iarg+1]);
      if (Pr_in < 0) error->all(FLERR,"Illegal collide_modify command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"mu_ratio") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal collide_modify command");
      mu_ratio = atof(arg[iarg+1]);
      if (mu_ratio < 0) error->all(FLERR,"Illegal collide_modify command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"k_ratio") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal collide_modify command");
      k_ratio = atof(arg[iarg+1]);
      if (k_ratio < 0) error->all(FLERR,"Illegal collide_modify command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"conserve") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal collide_modify command");
      if (strcmp(arg[iarg+1],"yes") == 0) conserve_energy = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) conserve_energy = 0;
      else error->all(FLERR,"Illegal collide_modify command");
      iarg += 2;
    } else error->all(FLERR,"Illegal collide_modify command");
  }
...
void Collide::collisions()
{
  ...
  if (!ambiflag) {
    if(bgk_mod > 0) {
      if(bgk_mod == 1) collisions_one_hybrid<1>();
      else collisions_one_hybrid<1>();
    } else if (nearcp == 0) {
...
template < int NEARCP > void Collide::collisions_one()
{
  ...
  for (int icell = 0; icell < nglocal; icell++) {
    ...
    // if on, limits collisions to nper_coll_lim collisions per particle
    // currently base it off global value (does not vary cell wise)
    int ncol = 0;
    int ncol_lim = 0;
    if(clim > 0) {
      ncol_lim = floor(clim*np);
    }
...
/* ----------------------------------------------------------------------
   NTC / BGK Hybrid for a single group
------------------------------------------------------------------------- */

template < int BGK > void Collide::collisions_one_hybrid()
{
  int i,j,k,m,n,ip,np;
  int nattempt,reactflag;
  int nattmax;
  double attempt,attmax,volume;
  Particle::OnePart *ipart,*jpart,*kpart;
  ...
  for (int icell = 0; icell < nglocal; icell++) {
    ...
    int npsq = np*np;
    double dnpsq = static_cast<double> (npsq);
    attmax = nattempt_lim*dnpsq;
    
    if(attempt >= attmax && bgk_mod > 0) {
      if(bgk_mod==1) relax_bgk(icell);
      else relax_esbgk(icell);
      //printf("ESBGK\n");
    } else {
      collide_dsmc(icell,nattempt);
      //printf("DSMC\n");
    }
  }
  //printf("done collide_one_hybrid\n");
}

/* ----------------------------------------------------------------------
   BGK algorithm for a single group
------------------------------------------------------------------------- */

void Collide::relax_bgk(int icell)
{
  // no BGK at the moments!
}

/* ----------------------------------------------------------------------
   ESBGK algorithm for a single group
------------------------------------------------------------------------- */

void Collide::relax_esbgk(int icell)
{
  int n;
  ...
  // Do we relax if there is one particle?
  //printf("mu_ratio:%.4e\n",mu_ratio);
  //printf("k_ratio:%.4e\n",k_ratio);
  //printf("Pr_sim:%.4e\n",Pr_sim);
  ...
  // Calculate bulk momentum and csq= vi.vj
  double vcom[3], sumu, sumv, sumw, sumvv, csq;
  sumu  = 0.0;
  sumv  = 0.0;
  sumw  = 0.0;
  sumvv = 0.0;
  ...
  for (int i = 0; i < np; i++) {
    ...
    sumv = sumv + v[1];
    sumw = sumw + v[2];
    sumvv = sumvv + v[0]*v[0] + v[1]*v[1] + v[2]*v[2];
  }
  ...
  double Vsq   = vcom[0]*vcom[0]+vcom[1]*vcom[1]+vcom[2]*vcom[2];
  int ip = cinfo[icell].first;
  double mass  = particle->species[particles[ip].ispecies].mass;
  double kb    = update->boltz;
  double temp  = mass*(csq - Vsq) / (3.0*kb); // Ta
  //printf("vcom_old:%.4e,%.4e,%.4e\n",vcom[0],vcom[1],vcom[2]);
  //printf("temp_old:%.4e\n",temp);
  ...
  // Store vars into local vars
  double weight = cinfo->weight;
  double volume = cinfo->volume;
  double fnum   = update->fnum;
  double dt     = update->dt;

  double oVol   = 1.0/volume*weight;
  double nrho   = np*fnum*oVol;
  //printf("icell:%i\n",icell);
  //printf("np:%.4e\n",np);
  //printf("nrho:%.4e\n",nrho);
  //printf("--------------------------------\n");
  ...
  double Pr = Pr_sim;
  double Pr_ratio = (1.0-Pr)/Pr;
  for (int i = 0; i < 6; i++) {
    pij[i] = pij[i] * Pr_ratio;
  }

  // Manually compute higher powers of pij
  // Expansion of sqrt(1-x)
  double pij2[6];
  pij2[0] = pij[0]*pij[0]+pij[3]*pij[3]+pij[4]*pij[4];
  pij2[1] = pij[3]*pij[3]+pij[1]*pij[1]+pij[5]*pij[5];
  pij2[2] = pij[4]*pij[4]+pij[5]*pij[5]+pij[2]*pij[2];
  pij2[3] = pij[0]*pij[3]+pij[3]*pij[1]+pij[4]*pij[5];
  pij2[4] = pij[0]*pij[4]+pij[3]*pij[5]+pij[4]*pij[2];
  pij2[5] = pij[3]*pij[4]+pij[1]*pij[5]+pij[5]*pij[2];

  double pij3[6];
  pij3[0] = pij2[0]*pij[0]+pij2[3]*pij[3]+pij2[4]*pij[4];
  pij3[1] = pij2[3]*pij[3]+pij2[1]*pij[1]+pij2[5]*pij[5];
  pij3[2] = pij2[4]*pij[4]+pij2[5]*pij[5]+pij2[2]*pij[2];
  pij3[3] = pij2[0]*pij[3]+pij2[3]*pij[1]+pij2[4]*pij[5];
  pij3[4] = pij2[0]*pij[4]+pij2[3]*pij[5]+pij2[4]*pij[2];
  pij3[5] = pij2[3]*pij[4]+pij2[1]*pij[5]+pij2[5]*pij[2];

  double pij4[6];
  pij4[0] = pij3[0]*pij[0]+pij3[3]*pij[3]+pij3[4]*pij[4];
  pij4[1] = pij3[3]*pij[3]+pij3[1]*pij[1]+pij3[5]*pij[5];
  pij4[2] = pij3[4]*pij[4]+pij3[5]*pij[5]+pij3[2]*pij[2];
  pij4[3] = pij3[0]*pij[3]+pij3[3]*pij[1]+pij3[4]*pij[5];
  pij4[4] = pij3[0]*pij[4]+pij3[3]*pij[5]+pij3[4]*pij[2];
  pij4[5] = pij3[3]*pij[4]+pij3[1]*pij[5]+pij3[5]*pij[2];

  double pij5[6];
  pij5[0] = pij4[0]*pij[0]+pij4[3]*pij[3]+pij4[4]*pij[4];
  pij5[1] = pij4[4]*pij[3]+pij4[1]*pij[1]+pij4[5]*pij[5];
  pij5[2] = pij4[4]*pij[4]+pij4[5]*pij[5]+pij4[2]*pij[2];
  pij5[3] = pij4[0]*pij[3]+pij4[3]*pij[1]+pij4[4]*pij[5];
  pij5[4] = pij4[0]*pij[4]+pij4[3]*pij[5]+pij4[4]*pij[2];
  pij5[5] = pij4[3]*pij[4]+pij4[1]*pij[5]+pij4[5]*pij[2];
  ...
  Sij[1] = Sij[1] + 1.0;
  Sij[2] = Sij[2] + 1.0;
  //printf("Sij:%.4e,%.4e,%.4e,%.4e,%.4e,%.4e\n",Sij[0],Sij[1],Sij[2],Sij[3],Sij[4],Sij[5]);

///////////////////////////////////////////////////////////////////////
  // Calculate relaxation probability

  int isp = particles[ip].ispecies;
  double omegal  = extract(isp,isp,"omega");
  double alphal  = extract(isp,isp,"alpha");
  double T_refl  = extract(isp,isp,"tref");
  double d_refl  = extract(isp,isp,"diam");
  ...
  double frq_ratio  = alphal*(5.0-2.0*omegal)*(7-2.0*omegal)/
                     (5.0*(alphal + 1.0)*(alphal + 2.0));

  double collfreq   = 4.0*d_refl*d_refl*frq_ratio*
                    nrho*sqrt(kb*T_refl*3.14159/mass)*
                    pow(temp/T_refl,1.0-omegal)*Pr_sim;

  double collprob    = 1.0-exp(-collfreq*dt);
  //printf("freq:%.4e,%.4e,%.4e\n",frq_ratio,collfreq,collprob);
  //printf("--------------------------------\n");

  // Calculate number to relax and randomize particle list
  if (collprob > 1.0) error->one(FLERR,"Relax Probability exceeds one");
  if (collprob < 0.0) error->one(FLERR,"Relax Probability negative");

  double vn[3];
  double vmp = sqrt(2.0 * kb * temp / mass);
  sumu = 0;
  sumv = 0;
  sumw = 0;
  sumvv = 0;
  for(int i = 0; i < np; i++) {
    ...
    if(random->uniform()<collprob)
    {
      ...
      for (int j = 0; j < 3; j++) {
         ...
         if(random->uniform()<0.5) vn[j] = A * cos(theta) * vmp;
         else vn[j] = A * sin(theta) * vmp;
         
      }
      ipart->v[0] = vn[0]*Sij[0] + vn[1]*Sij[3] + vn[2]*Sij[4] + vcom[0];
      ipart->v[1] = vn[0]*Sij[3] + vn[1]*Sij[1] + vn[2]*Sij[5] + vcom[1];
      ipart->v[2] = vn[0]*Sij[4] + vn[1]*Sij[5] + vn[2]*Sij[2] + vcom[2];
      ncollide_one++;
      //printf("v[0]:%.4e\n",v[0]);
    }
  ...
  if(conserve_energy) {
    ...
    vcom_new[2] = sumw / np;
    csq = sumvv/np;
    Vsq = vcom_new[0]*vcom_new[0]+vcom_new[1]*vcom_new[1]+vcom_new[2]*vcom_new[2];
    double temp_new  = mass*(csq - Vsq) / (3.0*kb); // Ta_new
    double theta_frac = sqrt(temp/temp_new);
    //printf("vcom_new:%.4e,%.4e,%.4e\n",vcom_new[0],vcom_new[1],vcom_new[2]);
    //printf("temp_new:%.4e\n",temp_new);
    //printf("--------------------------------\n");
    for (int i = 0; i < np; i++) {
      ipart = &particles[plist[i]];
      double *v = ipart->v;
      for (int j = 0; j < 3; j++) ipart->v[j] = (v[j] - vcom_new[j])*theta_frac + vcom[j];
    }
  }
  ...
  vcom[2] = sumw/np;
  csq = sumvv/np;
  Vsq = vcom[0]*vcom[0]+vcom[1]*vcom[1]+vcom[2]*vcom[2];

  double temp_new = mass/(3.*update->boltz)*(csq-Vsq);
...
/* ----------------------------------------------------------------------
   NTC algorithm for a single group (for hybrid method)
------------------------------------------------------------------------- */

void Collide::collide_dsmc(int icell, int nattempt)
{
  int i,j,n,ip,np;
  int nattmax;
  double attempt,volume;
